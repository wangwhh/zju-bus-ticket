...
<template>
    <div id="app">
        <a-card :bordered="false" style="height:fit-content; position: relative;">
            <p style="font-size: 25px; font-weight:450 ;text-align: left; margin-left: 10px; margin-top: 10px; margin-bottom: 5px;">{{getYearDay()}}</p>
            <div style="display: flex; justify-content: space-between; align-items: center; margin: 15px; margin-top: 0;">
                <div style="width: 33%;">
            ...
            <img :src="refreshImage" alt="刷新" class="right-image" />
            <p class="time-text">{{ getDay() }}</p>
            <p class="time-text">{{ currentTime }}</p>

            <p class="green-text">当前时段趟次</p>
            <!-- 显示二维码 -->
            <a-qrcode value="http://www.antdv.com" :color="token.colorInfoText" style="margin: auto; margin-bottom: 30px;"/>
